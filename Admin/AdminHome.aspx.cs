    ...
    public partial class AdminHome : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // Check if the admin is assigned to a college before performing operations
                Boolean assigned = Driver.VerifyAdminAssignedStatus(user.ID.ToString());
                if (assigned == false)
                {
                    Response.Redirect("../UnassignedAdmin.aspx", false);
                }
